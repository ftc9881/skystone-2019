...
public class OdometryExperiment {
	...
	//A is on the left-hand side, B is on the right-hand side
	...
	public static void main(String[] args) {
		...
		double[] t1 = new double[]{0, Math.PI/4, 0, 5};
		...
		double[] t2 = new double[]{0, -Math.PI/4, 5, 0};
		...
		double[] t3 = new double[]{0, -Math.PI/4, 4, -2};
	...
	//returns dX, dY
	public static Coord calcFB (double prevIMU, double currIMU, double dA, double dB) {
		//right is positive, left is negative
		double dDir = currIMU - prevIMU;
		double dX, dY;
		...
		if (Math.abs(dDir) < 0.01) {
			...
			dY = ((dA + dB) / 2.0) * Math.sin(prevIMU);
		//case making a turn: may not be neccessary, error reduction is probably minimal
		} else {
			//turn radius
			double r = ((dA + dB) / 2.0) * (1.0 / dDir);
					
			dX =  r*Math.cos(currIMU - (Math.PI/2)) - r*Math.cos(prevIMU - (Math.PI/2));
			dY =  r*Math.sin(currIMU - (Math.PI/2)) - r*Math.sin(prevIMU - (Math.PI/2));
		}
